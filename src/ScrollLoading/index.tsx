...
const ScrollLoading: FC<ScrollLoadingProps> & ScrollLoadingAPI = (props) => {
  const { CURRENT_PAGE_NUMBER } = ScrollLoading.DEFAULT_PROPS;
  const { params, emptyClassName } = props;
  const defaultCurrent = params?.current ?? CURRENT_PAGE_NUMBER; // 当前页码默认值
  ...
  return (
    ...
    >
      <div>
        <button type="button" onClick={getDataSource}>
          执行请求
        </button>
      </div>
